...
def test_load_rules_rejects_range_without_bounds(
...
def test_check_freshness_recent_passes() -> None:
    rule = FreshnessRule(
        id="f", type="freshness", severity="warning", description=None, max_age_days=7
    )
    now = datetime(2025, 1, 10, tzinfo=UTC)
    started = datetime(2025, 1, 9, tzinfo=UTC)
...
def test_check_freshness_stale_fails() -> None:
    ...
    now = datetime(2025, 1, 30, tzinfo=UTC)
...
def test_check_freshness_assumes_utc_for_naive_datetime() -> None:
    rule = FreshnessRule(
        id="f", type="freshness", severity="warning", description=None, max_age_days=1
    )
    now = datetime(2025, 1, 10, tzinfo=UTC)
